...
function HomeSyncButton({ t }) {
  ...
  const [last, setLast] = React.useState('Last synced 14 min. ago');
  const sync = () => { if (syncing) return; setSyncing(true); setTimeout(() => { setSyncing(false); setLast('Last synced just now'); }, 1600); };
  return (
    <button onClick={sync} style={{ display: 'flex', alignItems: 'center', gap: 14, width: '100%', textAlign: 'left', cursor: 'pointer', padding: '15px 18px', borderRadius: 20, border: 'none', background: t.gradient, boxShadow: `0 18px 36px -12px ${hexA(t.accent, 0.4)}` }}>
      <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 46, height: 46, flexShrink: 0, color: '#fff' }}>
        {syncing ? <span style={{ width: 20, height: 20, borderRadius: '50%', border: '2.5px solid rgba(255,255,255,.35)', borderTopColor: '#fff', animation: 'mspin .8s linear infinite' }} /> : <MIcon k="sync" size={22} />}
      </span>
      <span style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        <span style={{ fontFamily: FUI, fontSize: 15.5, fontWeight: 600, color: '#fff' }}>{syncing ? 'Syncing…' : 'Sync now'}</span>
        <span style={{ fontFamily: FUI, fontSize: 12, color: 'rgba(255,255,255,.78)' }}>{last}</span>
      </span>
    </button>
  );
}
    ...
    <div style={{ position: 'relative', flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
        ...
        {t.design === 'redesign' && (
          <div style={{ display: 'flex', gap: 9 }}>
            <button onClick={onJournal} style={{ flex: 1, minWidth: 0, textAlign: 'left', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', borderRadius: 16, background: t.surface, border: `1px solid ${t.line}` }}>
              <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 40, height: 40, borderRadius: 12, background: t.gradient, color: '#fff', flexShrink: 0 }}><MIcon k="pencil" size={17} /></span>
      ...
      {t.design === 'redesign' ? (
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, padding: '56px 16px 26px', background: `linear-gradient(to top, ${t.bg} 40%, ${hexA(t.bg, 0.85)} 62%, transparent)`, pointerEvents: 'none', display: 'flex', gap: 9 }}>
          <button onClick={onRecord} style={{ pointerEvents: 'auto', flex: 1, height: 56, borderRadius: 16, border: '1px solid rgba(255,255,255,.22)', background: t.gradient, color: '#fff', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, fontFamily: FD, fontSize: 16, fontWeight: 600, boxShadow: `0 12px 26px -8px ${hexA(t.accent, 0.6)}` }}>
        ...
        <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, height: 130, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', paddingBottom: 34, background: `linear-gradient(to top, ${t.bg} 34%, transparent)`, pointerEvents: 'none' }}>
          <button onClick={onRecord} aria-label="Record" style={{ pointerEvents: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'center', width: 68, height: 68, borderRadius: '50%', border: 'none', background: t.gradient, color: '#fff', cursor: 'pointer', boxShadow: `0 0 0 6px ${t.bg}, 0 12px 26px -6px ${hexA(t.accent, 0.6)}` }}>
...
function PhoneDetail({ t, r, onBack, openEditor }) {
      ...
      {renameSp && (
        <div style={{ position: 'absolute', inset: 0, zIndex: 40, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', background: 'rgba(6,5,12,.55)' }} onClick={() => setRenameSp(null)}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: t.bg, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 24, animation: 'msheet .28s cubic-bezier(.16,.84,.44,1)' }}>
            ...
            <button onClick={() => { setNames((n) => { const x = { ...n }; if (renameTxt.trim()) x[renameSp] = renameTxt.trim(); else delete x[renameSp]; return x; }); setRenameSp(null); }} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, width: '100%', fontFamily: FUI, fontSize: 15, fontWeight: 600, color: '#fff', padding: '13px 20px', borderRadius: 14, border: 'none', cursor: 'pointer', background: t.gradient, marginBottom: 10 }}><MIcon k="check" size={16} /> Save</button>
...
function PhoneJournalNew({ t, onBack, onDone }) {
  const [mode, setMode] = React.useState(null); // null | blank | notes
        ...
        {mode === 'notes' && <>
          ...
          {layout === 'ai' && <>
            <div style={{ position: 'relative', marginTop: 2 }}>
              ...
              <button title="Dictate the instructions" style={{ position: 'absolute', right: 9, bottom: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', width: 30, height: 30, borderRadius: '50%', border: 'none', cursor: 'pointer', background: t.gradient, color: '#fff' }}><MIcon k="micFill" size={13} /></button>
            </div>
  ...
  const pageCard = (key) => {
    ...
    if (key === 'today') return (
      <button key={key} onClick={onToday} style={{ textAlign: 'left', cursor: 'pointer', padding: 14, borderRadius: 16, background: t.surface, border: `1px solid ${t.hair}`, display: 'flex', alignItems: 'center', gap: 11, width: '100%' }}>
        <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 36, height: 36, borderRadius: 11, background: t.gradient, color: '#fff', flexShrink: 0 }}><MIcon k="pencil" size={16} /></span>
        <div style={{ flex: 1, minWidth: 0 }}>
            ...
            : summary
              ...
              : manual
                ? <>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <button style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 40, height: 40, borderRadius: '50%', border: 'none', cursor: 'pointer', background: t.gradient, color: '#fff', flexShrink: 0 }}><MIcon k="micFill" size={17} /></button>
                      <span style={{ flex: 1 }} />
  ...
  return (
    <div style={{ flex: 1, minHeight: 0, display: 'flex' }}>
      ...
      <div style={{ width: 320, flexShrink: 0, display: 'flex', flexDirection: 'column', background: t.bg2, borderRight: `1px solid ${t.line}` }}>
        <div style={{ padding: '16px 18px 10px' }}><SectionLabel text="Journal" t={t} /></div>
    ...
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', background: t.bg }}>
      {engineBar && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 16px', background: t.surface, borderBottom: `1px solid ${t.line}`, flexShrink: 0 }}>
          ...
          <span style={{ fontFamily: FUI, fontSize: 13, color: t.muted }}>Engines:</span>
          <span style={{ fontFamily: FM, fontSize: 12.5, color: t.text }}>On-device <span style={{ color: t.faint }}>→</span> OpenAI <span style={{ color: t.faint }}>→</span> ElevenLabs</span>
              ...
              return (
                <button key={r.id} onClick={() => setSel(r.id)} style={{ display: 'flex', alignItems: 'flex-start', gap: 11, width: '100%', textAlign: 'left', padding: 12, borderRadius: 13, marginBottom: 2, cursor: 'pointer', background: on ? hexA(t.accent, t.mode === 'dark' ? 0.14 : 0.08) : 'transparent', border: `1px solid ${on ? t.hair : 'transparent'}` }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 5 }}>
                    ...
                    <div style={{ fontFamily: FM, fontSize: 10.5, color: t.faint }}>{r.app} · {r.when}</div>
                  </div>
                </button>
              );
            })}
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '32px 40px' }}>
            <div style={{ maxWidth: 640 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 7, fontFamily: FM, fontSize: 11, fontWeight: 600, letterSpacing: '.11em', color: t.accentLite, marginBottom: 14 }}><MIcon k="spark" size={13} /> CLEANED UP ON-DEVICE</div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginTop: 30, padding: '18px 22px', background: t.surface, border: `1px solid ${t.line}`, borderRadius: 18 }}>
                <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 46, height: 46, borderRadius: '50%', background: t.gradient, color: '#fff', flexShrink: 0 }}><MIcon k="bolt" size={20} /></span>
...
function PhoneConversation({ t, onCancel, onDone, needsSetup }) {
  ...
  return (
    <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', background: t.bg2 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', paddingBottom: 2 }}>
        {window.ListeningGhost && (phase === 'listening' || phase === 'processing') && <ListeningGhost phase={phase === 'processing' ? 'note' : 'group'} size={90} />}
      </div>
      <div style={{ height: 70, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 24px 8px' }}>
        {phase === 'listening' && <Waveform t={t} color={t.accent} bars={34} height={70} />}
...
function StateHome({ t, empty, banner }) {
  ...
  return (
    <div style={{ position: 'relative', flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, height: 130, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', paddingBottom: 34, background: `linear-gradient(to top, ${t.bg} 34%, transparent)`, pointerEvents: 'none' }}>
        <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 72, height: 72, borderRadius: '50%', background: t.gradient, color: '#fff', boxShadow: `0 14px 26px -6px ${hexA(t.accent, 0.7)}` }}><MIcon k="micFill" size={28} /></span>
      </div>
    </div>
  );
}
...
Object.assign(window, { PhoneApp, AppleSplit, AppleJournal, WatchApp, RecRow, PhoneConversation, StateBanner, StateHome, OldDeviceView, HomeSyncButton, PhoneRecording, PhoneDetail, PhoneJournal, PhoneDigest });
