...
function PhoneSettings({ t, dark, setDark, onBack, initial, initialConsentFor, initialEraseOpen, initialPresetId }) {
  ...
  const pages = {
    models: { title: 'Models', body: (
      <>
        ...
        <div style={{ display: 'flex', alignItems: 'center', paddingLeft: 4 }}>
          <SectionLabel text="Remote connectors" t={t} /><span style={{ flex: 1 }} />{(chain[0] || {}).p === 'self'
        ...
        {engine && (
          <div style={{ display: 'flex', gap: 9 }}>
            <GhostBtn title={engine === 'self' ? 'Open server dashboard' : 'Manage account · ' + ENGINE_NAME[engine]} icon="arrowUR" t={t} style={{ flex: 1 }} />
            {engine !== 'self' && <GhostBtn title="Usage console" icon="globe" t={t} />}
          </div>
        )}
    ...
    github: { title: 'Sync to GitHub', body: (
      <>
        {infoCard('Mirror transcripts, journals, rendered rewrites and daily summaries to a Git repository — a plain-text backup you own.', (
        ...
        <SettGroup title="What syncs" t={t}>
          ...
          <SettRow icon="check" label="Journals" sub="Days, weeks and topic books" t={t} />
