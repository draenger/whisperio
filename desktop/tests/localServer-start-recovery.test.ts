...
describe('localServer — recovery from a failed start', () => {
  ...
  it('starts successfully on a retry once the binary is present', async () => {
    ...
    // main's READY_LINE_RE only accepts whisper.cpp's real ready line.
    proc.stdout.emit('data', 'whisper server listening at http://127.0.0.1:8178')
  ...
  it('does not pin the status at "starting" when the model is missing after a download', async () => {
    setPlatform('win32')
    const mod = await freshModule()
    // exe absent until the extract step "writes" it (downloadServerBinary
    // verifies the exe exists right after extraction), model file absent.
