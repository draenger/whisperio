...
describe('localServer', () => {
  ...
  describe('startServer failure paths reset the status so retries work', () => {
    ...
    it('resets when the model is missing after a successful download', async () => {
      ...
      // the powershell extract step calls back with no error AND "produces" the
      // exe -- the merged download path verifies extraction actually yielded the
      // binary before anything else runs, so the mock fs must reflect it.
      let exeExtracted = false
      mockExistsSync.mockImplementation((p: string) => {
        const s = String(p)
        if (s.endsWith('whisper-server.exe')) return exeExtracted
        return !s.endsWith('model.bin')
      })
      mockExecFile.mockImplementation((cmd: string, _args: unknown[], cb?: (err: Error | null) => void) => {
        if (cmd === 'powershell' && cb) setImmediate(() => { exeExtracted = true; cb(null) })
        return makeFakeProc()
      })
