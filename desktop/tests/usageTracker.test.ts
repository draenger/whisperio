...
describe('usageTracker', () => {
  ...
  describe('atomic save cleanup', () => {
    const tmpFiles = (): string[] => readdirSync(testDir).filter((f) => f.endsWith('.tmp'))
    const storeFiles = (): string[] => readdirSync(testDir).filter((f) => f === 'usage.json')
    const fsError = (code: string, message: string): Error => Object.assign(new Error(message), { code })

    it('removes the temp file when every rename retry fails, instead of leaking it into userData', async () => {
      // EPERM is a "transient" code, so this also exercises retry exhaustion.
      renameFailure = fsError('EPERM', 'EPERM: operation not permitted, rename')
      const errSpy = vi.spyOn(console, 'error').mockImplementation(() => {})

      // Fail-soft contract still holds: the caller sees no rejection...
      await expect(recordLLM({ provider: 'openai', inputTokens: 1000, outputTokens: 100 })).resolves.toBeUndefined()
    ...
    it('leaves no temp file behind on a successful save either', async () => {
      await recordSTT({ provider: 'openai', audioSeconds: 60 })
      expect(tmpFiles()).toEqual([])
      expect(storeFiles()).toHaveLength(1)
      expect(readdirSync(testDir)).toHaveLength(1)
    })

    it('keeps the previous usage.json intact when a later save fails', async () => {
      await recordLLM({ provider: 'openai', inputTokens: 1_000_000, outputTokens: 0 })
      const before = getUsage()

      renameFailure = fsError('ENOSPC', 'ENOSPC: no space left on device, rename')
      const errSpy = vi.spyOn(console, 'error').mockImplementation(() => {})
      await recordLLM({ provider: 'openai', inputTokens: 1_000_000, outputTokens: 0 })
      errSpy.mockRestore()
      renameFailure = null
